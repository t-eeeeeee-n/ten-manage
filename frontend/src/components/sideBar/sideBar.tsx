...
const SideBar: React.FC<SideBarProps> = ({ children, navigations, session }: SideBarProps) => {
    ...
    return(
        <>
            {session ? (
                <>
                    <aside className={`fixed duration-300 ease-in-out overflow-y-auto overflow-x-hidden min-w-[65px] shadow-md dark:shadow-zinc-500 ${menuOpen ? "w-[250px]" : "w-[60px]"}`} style={{ height: asideHeight }}>
                        <div className={"p-2"}>
              ...
              <span className={"flex justify-center"}>
                <IoSearchSharp className={`text-black ${menuOpen ? "mr-4" : "m-0"}`}/>
                                    ...
                                    <input id="header-search" placeholder="Search..." className={`bg-gray-200 border-none focus:outline-none ${menuOpen ? "visible w-full" : "invisible w-0"}`}/>
                                </div>
                            </div>
                            <span className={"block h-1 mb-6 bg-gray-200 w-full"}></span>
                            {navigations.map((navigation) => (
                                <Link href={navigation.path} key={navigation.page} className={`flex items-center h-10 ${isPageActive(navigation.path) ? "text-white bg-blue-700 duration-200" : "hover:text-white hover:bg-blue-500 duration-200 "}`}>
                ...
                <>{children}</> // Ensure children are wrapped in a fragment or another element
            )}
        </>
    )
}
